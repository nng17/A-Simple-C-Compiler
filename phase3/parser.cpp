...
{
    //cout << "inside variable declarator" << endl;
    ...
    if (lookahead == '[') {
        ...
        //cout << "variable declared" << name << endl;
    }
}
...
{
    //cout << "inside declaration" << endl;
    int spec = specifier();
    declarator(spec);

    while (lookahead == ',') {
        //cout << "found multiple declarator" << endl;
        match(',');
        declarator(spec);
    }
...
static void parameter(Parameters *p)
{
    ...
    string name = expect(ID);
    declareVar(name, Type(SCALAR,spec,ind));
    p->push_back(Type(SCALAR,spec,ind));
}
...
static void parameters(Parameters *p)
{
    int spec;
    if (lookahead == VOID) {
        spec = lookahead;
...
static void globalDeclarator(int spec)
{
    ...
    if (lookahead == '[') {
        ...
        parameters(p);
        match(')');
        declareFunction(name, Type(FUNCTION, spec, ind, p));
        openScope();
    }
    else
        declareVar(name, Type(SCALAR,spec,ind));
...
{
    //cout << "reached topLevelDeclaration" << endl;
    Parameters *p = new Parameters;
    int spec = specifier();
    unsigned ind = pointers();
    string name = expect(ID);

    //cout << name << '\t' << lookahead << endl;
    if (lookahead == '[') {
        match('[');
        int length = expectNum(NUM);
        match(']');
    ...
    else if (lookahead == '(')
    {
        ...
        {
            match('{');
            defineFunction(name, Type(FUNCTION, spec, ind, p));
            declarations();
            statements();
            closeScope();
            match('}');
